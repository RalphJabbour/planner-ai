...
class BehaviorModel:
    ...
    def get_or_create_profile(self, student_id: int) -> ProductivityProfile:
        """
        Gets the existing profile or creates a new one with default values
        """

        # Check if profile exists
        profile = self.db.query(ProductivityProfile).filter(
        ...
        new_profile = ProductivityProfile(
    ...
    def update_profile(self, student_id: int, force_update: bool = False) -> ProductivityProfile:
        """
        ...
        # Extract features
        slot_efficiencies = self.feature_extractor.extract_slot_efficiencies(student_id)
        ...
        # Update profile
        profile.slot_weights = slot_efficiencies
        ...
        profile.fatigue_factor = fatigue_params["fatigue_factor"]
        profile.recovery_factor = fatigue_params["recovery_factor"]
        profile.day_multipliers = adjustment_factors["day_multipliers"]
        profile.soft_obligation_buffer = adjustment_factors["soft_obligation_buffer"]
        profile.retention_rates = retention_rates
    ...
    def initialize_cold_start(self, student_id: int, preferences: Dict = None) -> ProductivityProfile:
        """
        ...
        profile.slot_weights = default_slot_weights
        profile.peak_windows = peak_windows
        
        # Use standard defaults for other parameters
        # (We already set defaults in get_or_create_profile)
